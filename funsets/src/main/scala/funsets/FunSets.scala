...
object FunSets {
   ...
   * that satisfies `p`.
   * Negate both the conditional p and the return value of f. 
   * The else if block in the forall function will return false if the set contains
   * some integer a that satifies p. Negate the false returned by this block to get true.
   */
